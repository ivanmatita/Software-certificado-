...
import { ArrowLeft, Printer, Save, RefreshCw, FileSignature, UserCircle2, Calendar, Gavel, X, Loader2 } from 'lucide-react';
    ...
    <div className="bg-slate-100 h-screen flex flex-col animate-in fade-in overflow-hidden">
      ...
      <div className="bg-slate-900 text-white p-4 flex justify-between items-center sticky top-0 z-50 shrink-0 shadow-xl print:hidden">
          ...
          <div className="flex gap-2">
              {!showEditor && (
                <button onClick={() => setShowEditor(true)} className="bg-white border-b-4 border-slate-400 text-slate-700 px-4 py-2 font-black uppercase text-xs hover:bg-slate-700 transition tracking-widest shadow-sm">
                    Editar Dados
                </button>
              )}
              <button onClick={() => window.print()} className="bg-white border-b-4 border-blue-600 text-slate-800 px-6 py-2 font-black uppercase tracking-tighter text-xs flex items-center gap-2 shadow-lg transition">
                  <Printer size={18}/> Imprimir A4
              </button>
              {showEditor && (
                <button onClick={handleSave} disabled={isSaving} className="bg-white border-b-4 border-blue-600 text-blue-800 px-8 py-2 font-black uppercase tracking-tighter text-xs flex items-center gap-2 shadow-xl transition disabled:opacity-50">
                    {isSaving ? <Loader2 className="animate-spin" size={18}/> : <Save size={18}/>} Gravar na Cloud
                </button>
              )}
          </div>
      </div>
      ...
      <div className="flex-1 flex overflow-hidden">
          ...
          {showEditor && (
            <div className="w-80 bg-white border-r border-slate-200 overflow-y-auto p-5 space-y-6 custom-scrollbar print:hidden animate-in slide-in-from-left duration-300">
                ...
                <button onClick={() => setShowEditor(false)} className="w-full py-3 bg-white border-b-4 border-slate-400 text-slate-600 rounded-none font-bold text-xs uppercase hover:bg-slate-50 transition tracking-widest">Fechar Editor</button>
            </div>
          )}
